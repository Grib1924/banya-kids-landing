...
const teachers = [
  ["/assets/teacher-new-1.jpg", "Помогу не бояться чистого листа и найти собственное решение — даже если сначала кажется, что «не получится» :)"],
  ["/assets/teacher-new-2.jpg", "Не исправляю работу за ученика. Задаю вопросы и показываю приёмы, чтобы ребёнок сам понял, как сделать лучше."],
  ["/assets/teacher-new-3.jpg", "На занятии можно пробовать, ошибаться, смеяться и снова пробовать. Именно так появляется настоящая самостоятельность."],
];
